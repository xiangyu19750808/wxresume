...
test('core flow: health → login → users/me → render → download → order', async () => {
  ...
  // 5) 下载
  {
    const r = await authedReq(`/v1/file/download?file_id=${encodeURIComponent(fileId)}`);
  ...
  // 7) 回调（paid）
  {
    const r = await authedReq(`/v1/order/callback?order_id=${encodeURIComponent(orderId)}&status=paid`);
    assert.equal(r.ok, true, 'order callback ok');
  }

  // 8) 查询状态
  {
    const r = await authedReq(`/v1/order/status?id=${encodeURIComponent(orderId)}`);
