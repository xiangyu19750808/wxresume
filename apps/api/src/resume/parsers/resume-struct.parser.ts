...
function buildSections(text: string): Map<string, string[]> {
...
function extractTopTokens(text: string, limit = 30): string[] {
...
function parseSkills(sectionText: string, fullText: string): string[] {
...
function parseCertificates(sectionText: string, fullText: string): string[] {
...
function parseLanguages(sectionText: string, fullText: string): LanguageEntry[] {
  const text = `${sectionText}\n${fullText}`;
...
export class ResumeStructParser {
  parse(resumeText: string): { resumeParsed: ResumeParsed; warnings: Warning[] } {
