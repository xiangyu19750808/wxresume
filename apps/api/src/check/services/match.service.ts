...
export async function calcMatchScore(resumeText: string, parsedJD: ParsedJD): Promise<MatchScoreResult> {
  ...
  const hardCheckItems: MatchScoreResult['hardCheckItems'] = [];
...
export class MatchService {
  async calculateMatchScore(resumeText: string, parsedJD: ParsedJD): Promise<number> {
    const { matchScore } = await calcMatchScore(resumeText, parsedJD);
    return matchScore;
  }
}
